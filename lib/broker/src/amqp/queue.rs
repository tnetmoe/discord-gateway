...
use amqprs::channel::{Channel, QueueDeclareArguments, QueueBindArguments};
use log::{debug, error};
...
pub async fn create_queue(channel: &Channel, queue_name: &str) -> Result<(String, u32, u32)> {
    let args = QueueDeclareArguments::durable_client_named(
        queue_name
    );

    match channel.queue_declare(args).await {
        Ok(Some(queue)) => {
            debug!("Declared queue: {}; Messages: {}; Consumers: {}", queue_name, queue.1, queue.2);
            Ok(queue)
        },
        Ok(None) => {
...
pub async fn bind_queue(channel: &Channel, queue: String, exchange: String, routing_key: String) -> Result<()> {
